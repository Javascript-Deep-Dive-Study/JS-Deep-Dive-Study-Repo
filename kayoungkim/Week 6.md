...
console.log(x); // 1
```
- if 문의 코드 블록이 실행되면 블록 레벨 스코프를 생성. 이를 위해 선언전 환경 레코드를 갖는 렉시컬 환경을 새롭게 생성하여 기존의 전역 렉시컬 환경을 교체해야 함.
- 새롭게 생성된 if 문의 코드 블록을 위한 렉시컬 환경의 외부 렉시컬 환경에 대한 참조는 if 문이 실행되기 이전의 전역 렉시컬 환경을 가리키고, 
- if 문이 종료되면 if 문의 코드 블록이 실행되기 이전의 렉시컬 환경으로 되돌림 (블록 레벨 스코프를 생성하는 모든 블록문에 적용됨)
* for 문의 변수 선언문에 let 키워드를 사용한 for 문은 코드 블록이 반복해서 실행될 때마다 코드 블록을 위한 새로운 렉시컬 환경을 생성. for 문의 코드 블록이 반복해서 실행될 때마다 독립적인 렉시컬 환경을 생성하여 식별자 값 유지
...
outerFunc();
```
- 중첩함수(inner)의 상위 스코프는 외부 함수의 스코프이므로 중첩함수 내부에서 자신을 포함하고 있는 외부 함수의 x 변수에 접근할 수 있다.
```javascript
const x = 1;
...
outerFunc();
```
- 그러나 outer 함수 내부에서 정의된 중첩함수가 아니라면 inner 함수를 내부에서 호출한다 하더라도 outer 함수의 변수에 접근할 수 없다.
- 이는 자바스크립트가 **렉시컬 스코프를 따르는 프로그래밍 언어**이기 때문이다.
...
## 클로저의 활용
- 클로저는 자신이 생성될 때의 환경을 기억해야하므로 메모리 차원에서 손해를 볼 수 있지만 자바스크립트의 강력한 기능이므로 적극적으로 사용해야 한다.
### 상태 유지
